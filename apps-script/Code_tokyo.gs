...
const TRIP_CURRENCIES = ['JPY', 'SGD', 'MYR']; // default currency = first
...
// All from->to pairs across the trip's currencies. e.g. for [JPY,SGD,MYR] this
// produces JPYSGD, SGDJPY, JPYMYR, MYRJPY, SGDMYR, MYRSGD.
...
function seedSettingsIfEmpty_(ss) {
  ...
  const rows = [
    ...
    ['fund_currency',         '',                                 'Currency of fund contributions, e.g. JPY. Leave blank to disable.'],
  ];
...
function doGet(e) {
  return handle_(e, (params) => {
    ...
    if (action === 'settlement') return computeSettlement_(params.currency || 'JPY');
